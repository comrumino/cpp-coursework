...
const Layer::PlacedGraphicCollection &Layer::getGraphicCollection() const { return graphics; }
...
tinyxml2::XMLNode* elementToXML(const framework::Element &framework_elmnt, tinyxml2::XMLDocument &doc) {
    tinyxml2::XMLElement* elmnt = doc.NewElement(framework_elmnt.getName().c_str()); 
    for (auto attr : framework_elmnt.getAllAttributes()) {
        elmnt->SetAttribute(attr.getName().c_str(), attr.getStrValue().c_str());
    }
    return elmnt;
}
...
framework::Element writeVectorGraphic(const HVectorGraphic &vectorGraphic) {
    ...
    vg_elmnt.addAttribute(framework::Attribute("closed", "false"));
