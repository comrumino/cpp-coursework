...
WordFinder::WordFinder(const Dictionary &dict, const Board &board,
...
TWordsList WordFinder::FindWords() {
  ...
  for (auto i = 0; i < mBoard.NumRows(); ++i) {
    tstr = "";
    for (auto j = 0; j < std::min(mBoard.NumCols(), mBoard.NumRows() - i);
         ++j) {
      tstr += mBoard(i + j, j);
    }
    tfrags = FragString(tstr, mMaxFoundWordLength);
  ...
  for (auto j = 1; j < mBoard.NumCols(); ++j) {
    tstr = "";
    for (auto i = 0; i < std::min(mBoard.NumCols() - j, mBoard.NumRows());
         ++i) {
      tstr += mBoard(i, j + i);
    }
    tfrags = FragString(tstr, mMaxFoundWordLength);
