...
unsigned int Dictionary::Size() { return mWords.size(); }

bool Dictionary::Contains(const std::string &word) const { return mWords.find(word) != mWords.end(); }
...
citerator Dictionary::begin() const { return mWords.cbegin(); }

citerator Dictionary::end() const { return mWords.cend(); }

citerator &Dictionary::FindNextWord(int size, citerator &iter) const { return iter; }
